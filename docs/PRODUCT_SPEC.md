...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| `FR-NOTIFY-01` | 기한 경과, D-3, D-1, 당일을 기본 알림 기준으로 제공한다. | P0 |
...
| `FR-NOTIFY-05` | 공통 앱바 알림함에서 기한 경과·당일 재료와 개봉 여부를 함께 확인하고 상태를 바로 기록한다. | P0 |
| `FR-NOTIFY-06` | 미개봉 상태로 기한이 지난 재료가 있으면 다음 구매량을 줄이거나 구매를 쉬는 피드백을 제공한다. | P1 |
